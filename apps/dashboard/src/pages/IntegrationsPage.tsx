...
function webhookUrl(id: string) {
  return `${window.location.origin}/api/v1/integrations/${id}/webhook`
}

// ── Run History Panel ──────────────────────────────────────────────────────────

function RunHistory({ integrationId, onClose }: { integrationId: string; onClose: () => void }) {
  const { data: runs, isLoading, error } = useQuery({
    queryKey: ['integration-runs', integrationId],
    queryFn: () => api.integrations.getRuns(integrationId),
    staleTime: 10_000,
  })
...
function UploadModal({
  ...
  const mutation = useMutation({
    mutationFn: (file: File) => api.integrations.uploadBatch(integration.id, file),
    onSuccess: (data) => {
  ...
  const accept = integration.connector_type === 'batch_csv' ? '.csv' : '.json,.ndjson'

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-md bg-j-surface border border-j-border rounded-lg overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-j-border">
          <span className="font-mono text-xs font-semibold text-j-bright">Upload to {integration.name}</span>
        ...
        <div className="px-4 py-4 space-y-3">
          <p className="font-mono text-[11px] text-j-dim">
            Upload a {integration.connector_type === 'batch_csv' ? 'CSV' : 'JSON'} file (max 50 MB).
...
function CreateModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-lg bg-j-surface border border-j-border rounded-lg overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-j-border">
          <span className="font-mono text-xs font-semibold text-j-bright">New Integration</span>
          <button onClick={onClose} className="font-mono text-[10px] text-j-dim hover:text-j-accent">✕</button>
        </div>
...
function IntegrationCard({
  ...
  const glyphColor = TYPE_COLOR[integration.connector_type] ?? 'text-j-dim'
    ...
    <div className="border border-j-border rounded bg-j-surface overflow-hidden hover:border-j-border-b transition-colors">
      ...
      <div className="px-4 pt-4 pb-3 border-b border-j-border flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <div className="font-mono text-j-bright font-medium truncate">{integration.name}</div>
            {integration.description && (
              <p className="font-mono text-[11px] text-j-dim mt-0.5 truncate">{integration.description}</p>
            )}
          </div>
        </div>
      ...
      <div className="px-4 py-3 flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2">
          <span className="font-mono text-[10px] tracking-[0.1em] uppercase text-j-dim">{integration.connector_type}</span>
          <span className="font-mono text-[10px] text-j-dim opacity-50">
            {fmtDate(integration.created_at)}
          </span>
        </div>
    ...
    <div className="flex-1 overflow-auto p-6 bg-j-bg">
      ...
      {!isLoading && !error && !(integrations ?? []).length && (
        <div className="text-center py-16">
          <p className="font-mono text-[11px] text-j-dim mb-3">no integrations yet</p>
          <button
            onClick={() => setShowCreate(true)}
